...
export const ReportModal: React.FC<ReportModalProps> = ({
  ...
  const totalBalanceSum = cycleTransactions.reduce(
    ...
    try {
      ...
      if (action === "download") {
        ...
        if (blob) {
          ...
          try {
            // ClipboardItem may be undefined in some browsers/environments
            if (navigator.clipboard && (window as any).ClipboardItem) {
              const ItemConstructor = (window as any).ClipboardItem;
              const item = new ItemConstructor({ "image/png": blob });
              await navigator.clipboard.write([item]);
              toast.success("Đã copy ảnh vào bộ nhớ tạm");
            } else if (navigator.clipboard && navigator.clipboard.writeText) {
              // Fallback: copy the image as a data URL (useful when image clipboard isn't supported)
              const dataUrl = await blobToDataURL(blob);
              await navigator.clipboard.writeText(dataUrl);
              toast.success(
                "Không hỗ trợ clipboard ảnh — đã copy data URL thay thế"
              );
            } else {
              ...
              toast.success(
                "Trình duyệt không hỗ trợ clipboard — đã tải ảnh xuống"
              );
            }
          } catch (err) {
            console.error("Capture/clipboard fallback failed:", err);
            toast.error(
              "Không thể copy ảnh vào clipboard. Vui lòng thử lưu ảnh."
            );
          }
        } else {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4 overflow-y-auto">
      ...
      <div className="relative z-[70] w-full max-w-[1300px] bg-white md:rounded-xl shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-200 flex flex-col max-h-[98vh]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-100/50 p-4 md:p-6">
          ...
          <div
            ref={reportRef}
            className="bg-white p-6 md:p-8 rounded-none md:rounded-xl shadow-sm border border-slate-200 flex flex-row gap-6 min-w-[1100px] items-start font-sans"
          >
                ...
                <div className="p-3 bg-white space-y-2">
                  ...
                  {Object.entries(shareStats.shareholderTotals).map(
                    ([name, amount]) => (
                      <div
                        key={name}
                        className="flex justify-between items-center px-3 py-2 rounded-lg border border-slate-100 bg-slate-50/50"
                      >
                        <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0 border rounded-lg border-slate-200 overflow-hidden">
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
                  <tr>
                    <th className="py-3 px-3 text-center w-[50px] border-r border-slate-100">
                      #
                    </th>
                    <th className="py-3 px-3 text-center w-[120px] border-r border-slate-100">
                      Ngày
                    </th>
                    <th className="py-3 px-3 text-right w-[120px] border-r border-slate-100">
                      Tổng dư
                    </th>
                    {/* REMOVED DIVIDEND COLUMN */}
                    <th className="py-3 px-3 text-right w-[120px] border-r border-slate-100 text-orange-600">
                      Anh Thảo
                    </th>
                    ...
                    cycleTransactions.map((t, idx) => {
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-slate-50">
                          <td className="py-3 px-3 text-center text-slate-400 font-medium border-r border-slate-100">
                            {idx + 1}
                          </td>
                          <td className="py-3 px-3 text-center font-bold text-slate-700 border-r border-slate-100">
                            {t.date}
                          </td>
                          <td className="py-3 px-3 text-right font-black text-slate-900 border-r border-slate-100">
