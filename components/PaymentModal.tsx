...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const visibleTransactions = transactions.filter((t) => {
    if (editingCycle) {
      ...
      return (
        ...
        t.paymentMonth === editingCycle.id
      );
    }
  ...
  const handleMonthChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedMonth(m);
    if (!note || note.trim() === "" || note.startsWith("Thanh toán kỳ")) {
      setNote(`Thanh toán kỳ ${m}/${selectedYear}`);
    }
  };
  ...
  const handleYearChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedYear(y);
    if (!note || note.trim() === "" || note.startsWith("Thanh toán kỳ")) {
      setNote(`Thanh toán kỳ ${selectedMonth}/${y}`);
    }
  };
  ...
  const selectedTransactions = visibleTransactions.filter((t) =>
    selectedIds.has(t.id)
  );

  const totalRemaining = selectedTransactions.reduce(
    ...
    selectedTransactions.forEach((t) => {
      ...
      if (bus && bus.isShareholding) {
        ...
        bus.shareholders?.forEach((sh) => {
          ...
          shareholderTotals[sh.name] =
            (shareholderTotals[sh.name] || 0) + shAmount;
      ...
      <div className="relative z-[70] w-full max-w-5xl bg-white md:rounded-xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-200 h-[100dvh] md:h-auto md:max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs md:text-sm text-slate-500 font-medium hidden md:block">
                {editingCycle
                  ? "Thêm hoặc bớt các bản ghi cho kỳ này"
                  : "Chọn các bản ghi để chốt sổ"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row relative">
          ...
          <div className="w-full md:w-[360px] bg-slate-50 p-2 md:p-5 flex flex-col gap-1 md:gap-3 shadow-[-5px_0_15px_-3px_rgba(0,0,0,0.05)] z-20 shrink-0 order-first md:order-last border-b md:border-b-0 border-slate-200 overflow-y-auto custom-scrollbar">
            {/* Payment Cycle Info */}
            <div className="relative overflow-hidden rounded-lg md:rounded-xl bg-gradient-to-br from-slate-900 to-slate-800 p-2.5 md:p-4 text-white shadow-lg shrink-0 flex flex-col justify-center">
              ...
              <div className="relative z-10 flex flex-col gap-1 opacity-90 md:opacity-100">
                <div className="flex items-center gap-2 text-slate-300">
                  ...
                  {editingCycle && (
                    <Lock size={12} className="text-orange-300 ml-1" />
                  )}
                </div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <div className="relative">
                      <select
                        ...
                        className="bg-white/10 hover:bg-white/20 border border-white/20 rounded-md py-0.5 pl-2 pr-6 text-sm font-bold text-white focus:outline-none focus:bg-slate-800 focus:border-white/40 cursor-pointer appearance-none text-center transition-colors"
                      >
            ...
            <div className="flex flex-col gap-2 shrink-0">
              {/* UPDATED: Compact Shareholding Breakdown matching Request */}
              <div className="rounded-xl border border-slate-200 overflow-hidden bg-white shadow-sm">
                {/* Header */}
                <div className="bg-slate-900 px-3.5 py-1 flex justify-between items-center text-white">
                ...
                <div className="p-3 bg-white space-y-2">
                  {/* Row 1: Chia cổ phần (Owner Share) */}
                  <div className="flex justify-between items-center px-3 py-1 rounded-lg border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                    ...
                    <span className="font-bold text-base text-slate-900">
                      {formatCurrency(shareStats.ownerTotal)}
                    </span>
                  </div>

                  {/* Row 2+: Shareholders */}
                  {Object.entries(shareStats.shareholderTotals).map(
                    ([name, amount]) => (
                      <div
                        key={name}
                        className="flex justify-between items-center px-3 py-1 rounded-lg border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors"
                      >
                        <div className="flex items-center gap-3">
              ...
              <table className="w-full text-sm text-left table-fixed">
                <thead className="bg-slate-50 text-slate-500 font-semibold sticky top-0 z-10 shadow-sm">
                  <tr>
                    ...
                    <th className="h-10 px-2 text-center hidden md:table-cell w-[140px]">
                      Trạng thái
                    </th>
                  </tr>
                </thead>
                  ...
                  {visibleTransactions.length === 0 ? (
                    ...
                    visibleTransactions.map((t) => {
                      ...
                      const isInCurrentCycle =
                        editingCycle?.transactionIds.includes(t.id);

                      // Calculate row shares
                      const { ownerShare, heldShare, isShareholding } =
                        getRowShares(t);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-2 py-3 text-center font-medium text-slate-700 truncate">
                            {t.date}
                          </td>
                          ...
                          <td className="px-2 py-3 text-right text-orange-600 font-medium">
                            ...
                            {isShareholding && heldShare !== 0
                              ? formatCurrency(heldShare)
                              : "-"}
                          </td>
