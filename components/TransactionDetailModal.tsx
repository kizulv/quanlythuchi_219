...
export const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
  ...
  const [conflictTransaction, setConflictTransaction] =
    useState<Transaction | null>(null);
  const [showSimpleDuplicateAlert, setShowSimpleDuplicateAlert] =
    useState(false);
  ...
  useEffect(() => {
    const loadBuses = async () => {
      ...
      if (!transaction.id && !breakdown.busId) {
        ...
        const mainBusId = defaultMainBus
          ? defaultMainBus.licensePlate
          : data[0]?.licensePlate || "";

        const defaultAccompanyingBus = data.find(
          (b) => b.licensePlate !== mainBusId
        );

        setBreakdown((prev) => ({
          ...prev,
          busId: mainBusId,
          partnerBusId: defaultAccompanyingBus
            ? defaultAccompanyingBus.licensePlate
            : prev.partnerBusId,
        }));
      }
    };
  ...
  useEffect(() => {
    if (transaction) {
      ...
      if (
        currentBreakdown.otherRevenueItems &&
        currentBreakdown.otherRevenueItems.length > 0
      ) {
        ...
        initRev = [
          {
            id: "legacy-rev-1",
            description: "Thu khác (cũ)",
            amount: currentBreakdown.revenueOther,
          },
        ];
      }
      ...
      if (
        currentBreakdown.otherExpenseItems &&
        currentBreakdown.otherExpenseItems.length > 0
      ) {
        ...
        initExp = [
          {
            id: "legacy-exp-1",
            description: "Chi khác (cũ)",
            amount: currentBreakdown.expenseOther,
          },
        ];
      }
      ...
      if (
        currentBreakdown.privateExpenseItems &&
        currentBreakdown.privateExpenseItems.length > 0
      ) {
        ...
        initPriv = [
          {
            id: "legacy-priv-1",
            description: "Chi riêng (cũ)",
            amount: transaction.privateExpense,
          },
        ];
      }
      ...
      const totalRevenueCalc =
        currentBreakdown.revenueDown +
        currentBreakdown.revenueUp +
        initRev.reduce((s, i) => s + i.amount, 0);
      const totalExpenseCalc =
        currentBreakdown.expenseFuel +
        currentBreakdown.expenseFixed +
        currentBreakdown.expensePolice +
        currentBreakdown.expenseRepair +
        initExp.reduce((s, i) => s + i.amount, 0);
      ...
      const autoNote = generateAutoDetails(
        initRev,
        initExp,
        initPriv,
  ...
  // --- CALCULATIONS ---
  const totalRevenueOther = otherRevenues.reduce(
    (sum, item) => sum + item.amount,
    0
  );
  const totalExpenseOther = otherExpenses.reduce(
    (sum, item) => sum + item.amount,
    0
  );

  // Always calculate private expenses, regardless of shared mode
  const totalPrivateExpense = privateExpenses.reduce(
    (sum, item) => sum + item.amount,
    0
  );
  ...
  const handleVariableExpenseChange = (
    field: keyof TransactionBreakdown,
    value: number
  ) => {
    setBreakdown((prev) => ({ ...prev, [field]: value }));
  ...
  const updateOtherRevenueItem = (
    id: string,
    field: keyof OtherRevenueItem,
    value: any
  ) =>
    setOtherRevenues(
  ...
  const updatePrivateExpenseItem = (
    id: string,
    field: keyof PrivateExpenseItem,
    value: any
  ) =>
    setPrivateExpenses(
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        setImageUrl(preview);
        const processedUrl = await processAndUploadImage(
          file,
          transaction.date
        );
  ...
  const proceedWithSave = () => {
    ...
    const newAutoNoteString = generateAutoDetails(
      otherRevenues,
      otherExpenses,
      privateExpenses,
      breakdown.isShared
  ...
  const renderCalendar = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      const isSelected =
        isCurrentMonthSelected && selectedDateObj.getDate() === d;
      ...
      days.push(
        <button
          ...
          className={`h-9 w-9 rounded-md flex items-center justify-center text-sm transition-all ${
            isSelected
      ...
      >
        ...
        <div className="grid grid-cols-7 mb-2">
          {VN_WEEKDAYS.map((day) => (
            <div
              key={day}
              className="h-9 w-9 flex items-center justify-center text-[10px] font-medium text-slate-400"
            >
              {day}
            </div>
          ))}
  ...
  const gridClass =
    "grid grid-cols-[1fr_90px] md:grid-cols-[1fr_130px] gap-2 items-center";
  const dynamicGridClass =
    "grid grid-cols-[24px_1fr_90px] md:grid-cols-[28px_1fr_130px] gap-2 items-center";
          ...
          <div className="w-full h-56 shrink-0 md:h-full md:w-6/12 bg-slate-900 relative flex-col group border-b md:border-b-0 md:border-r border-slate-800">
            ...
            <div className="flex-1 w-full h-full flex items-center justify-center p-4 bg-slate-950 relative overflow-hidden">
              {imageUrl ? (
                ...
                <div className="relative w-full h-full bg-slate-900 md:rounded-lg overflow-hidden flex flex-col items-center justify-center text-slate-500 border-2 border-dashed border-slate-800">
                  <ImageIcon
                    size={32}
                    className="mb-2 opacity-50 md:w-12 md:h-12"
                  />
            ...
            <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-3 z-10 px-4 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              ...
              <Button
                variant="primary"
                size="sm"
                className={`shadow-xl bg-white text-slate-900 hover:bg-slate-100 border-0 font-medium h-8 md:h-9 text-sm ${
          ...
          <div className="w-full md:w-6/12 bg-white flex flex-col flex-1 h-full text-slate-900 min-h-0">
            ...
            <div className="px-3 md:px-4 pt-3 pb-0 bg-white sticky top-0 z-10 shrink-0">
              ...
              <div className="flex items-center justify-between pb-3 md:pb-4 border-b border-slate-100 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                <div className="relative">
                  <div
                    className={`flex items-center gap-2 md:gap-3 border shadow-sm rounded-lg md:rounded-xl px-3 py-2 md:px-4 md:py-3 transition-all duration-200 min-w-[180px] md:min-w-[220px] select-none ${
                      isPaid
                    ...
                    onClick={() =>
                      !isPaid && setIsCalendarOpen(!isCalendarOpen)
                    }
                  >
                    <div
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] md:text-[11px] uppercase font-bold text-slate-400 tracking-wider leading-tight mb-0.5">
                        Ngày ghi sổ
                      </span>
                      <span
                        className={`text-xs md:text-[13px] font-bold leading-tight ${
                          date ? "text-slate-800" : "text-slate-400 italic"
                        }`}
                      >
                ...
                <div className="flex flex-col items-end justify-center">
                  <span className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 hidden md:block">
                    Đơn vị tính
                  </span>
                  <span className="text-xs italic lowercase text-slate-500">
                    nghìn đồng
                  </span>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 custom-scrollbar">
              ...
              <section className="bg-slate-50/80 rounded-lg border border-slate-200 p-2 md:p-3 space-y-2 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                      Xe chính
                    </label>
                    <select
                  ...
                  <div className={`space-y-1 transition-all duration-200`}>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setBreakdown({
                          ...breakdown,
                          partnerBusId: e.target.value,
                        })
                      }
                      disabled={isPaid}
                    >
                      {buses
                        .filter((b) => b.licensePlate !== breakdown.busId)
                        .map((b) => (
                          <option key={b.id} value={b.licensePlate}>
                            {b.licensePlate}
                          </option>
              ...
              <section className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <InputRow
                    label="Chiều xuôi"
                    value={breakdown.revenueDown}
                    onChange={(v) => handleRevenueChange("revenueDown", v)}
                    readOnly={isPaid}
                  />
                  <InputRow
                    label="Chiều ngược"
                    value={breakdown.revenueUp}
                    onChange={(v) => handleRevenueChange("revenueUp", v)}
                    readOnly={isPaid}
                  />
                  <div className="space-y-1.5">
                    {otherRevenues.map((item) => (
                      <div key={item.id} className={dynamicGridClass}>
                        <button
                          onClick={
                            isPaid
                              ? undefined
                              : () => removeOtherRevenueItem(item.id)
                          }
                        ...
                        <SmartInput
                          value={item.amount}
                          onCommit={(val) =>
                            updateOtherRevenueItem(item.id, "amount", val)
                          }
              ...
              <section className="space-y-1.5 pt-1">
                <div className={`${gridClass} pb-1 border-b border-slate-100`}>
                  ...
                  <SmartInput
                    ...
                    className={`w-full text-right h-8 rounded border border-red-200 px-2 text-xs md:text-sm font-bold text-red-600 outline-none transition-all ${
                      isPaid
                        ? "bg-slate-50 cursor-not-allowed"
                        : "bg-red-50/50 focus:border-red-400 focus:ring-1 focus:ring-red-100"
                    }`}
                    readOnly={isPaid}
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  <InputRow
                    ...
                    onChange={(v) =>
                      handleVariableExpenseChange("expenseFuel", v)
                    }
                    readOnly={isPaid}
                  />
                  ...
                  <div className={`${gridClass} group`}>
                    <div className="flex items-center gap-2">
                      <span className="text-xs md:text-sm text-slate-600 group-hover:text-slate-900 truncate">
                        Chi cố định
                      </span>
                    </div>
                  ...
                  <InputRow
                    ...
                    onChange={(v) =>
                      handleVariableExpenseChange("expensePolice", v)
                    }
                    readOnly={isPaid}
                  />
                  <InputRow
                    ...
                    onChange={(v) =>
                      handleVariableExpenseChange("expenseRepair", v)
                    }
                    readOnly={isPaid}
                  />
                      ...
                      <div key={item.id} className={dynamicGridClass}>
                        <button
                          onClick={
                            isPaid
                              ? undefined
                              : () => removeOtherExpenseItem(item.id)
                          }
                          className={`p-1 md:p-1.5 rounded transition-colors flex items-center justify-center ${
                            isPaid
                              ? "text-slate-300 cursor-not-allowed"
                        ...
                        <SmartInput
                          value={item.amount}
                          onCommit={(val) =>
                            updateOtherExpenseItem(item.id, "amount", val)
                          }
                          className="w-full text-right h-8 rounded border border-slate-200 bg-white px-2 text-xs md:text-sm font-medium text-slate-700 focus:border-slate-400 focus:ring-1 focus:ring-slate-100 outline-none transition-all"
                          readOnly={isPaid}
                        />
              ...
              <section className="bg-slate-50 rounded-lg border border-slate-200 p-2 md:p-3 shadow-sm">
                <h3 className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">
                  Tổng kết
                </h3>
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5 animate-in fade-in zoom-in-95 duration-200">
                    <div className={gridClass}>
                      ...
                      <div className="w-full h-8 flex items-center justify-end px-2">
                        <span className="font-bold text-xs md:text-sm text-slate-800">
                          {formatForDisplay(totalPrivateExpense)}
                        </span>
                      </div>
                    </div>
                        ...
                        <div key={item.id} className={dynamicGridClass}>
                          <button
                            onClick={
                              isPaid
                                ? undefined
                                : () => removePrivateExpenseItem(item.id)
                            }
                            className={`p-1 md:p-1.5 rounded transition-colors flex items-center justify-center ${
                              isPaid
                                ? "text-slate-300 cursor-not-allowed"
                          ...
                          <SmartInput
                            value={item.amount}
                            onCommit={(val) =>
                              updatePrivateExpenseItem(item.id, "amount", val)
                            }
                            className="w-full text-right h-8 rounded border border-slate-200 bg-white px-2 text-xs md:text-sm font-medium text-slate-700 focus:border-slate-400 focus:ring-1 focus:ring-slate-100 outline-none transition-all"
                            readOnly={isPaid}
                          />
                  ...
                  {selectedBus &&
                    (selectedBus.sharePercentage < 100 ||
                      (selectedBus.shareholders &&
                        selectedBus.shareholders.length > 0)) && (
                        ...
                        <div className="space-y-1.5 bg-white/50 border border-slate-100 p-2 rounded-lg">
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-600 font-bold">
                              Cổ phần ({selectedBus.sharePercentage}%)
                            </span>
                            <span className="text-slate-600 font-bold">
                              {formatForDisplay(
                                (remainingBalance *
                                  selectedBus.sharePercentage) /
                                  100
                              )}
                            </span>
                          </div>
                          {selectedBus.shareholders?.map((sh) => (
                            ...
                            >
                              <span className="text-slate-600 font-bold">
                                {sh.name} (Cầm hộ {sh.percentage}%)
                              </span>
                              <span className="font-bold text-slate-600">
                                {formatForDisplay(
                                  (remainingBalance * sh.percentage) / 100
                                )}
                              </span>
                            </div>
                          ))}
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-600">
                  Ghi chú thêm {isPaid && "(Có thể chỉnh sửa)"}
                </label>
                <textarea
            ...
            <div className="p-3 md:p-4 border-t border-slate-100 bg-white flex justify-between gap-3 z-10 shadow-[0_-1px_2px_rgba(0,0,0,0.03)] shrink-0">
              {transaction.id && (
                <Button
                  variant="outline"
                  className={`border-red-100 text-red-600 h-10 px-3 ${
                    isPaid
                  ...
                  title={
                    isPaid
                      ? "Không thể xóa bản ghi đã thanh toán"
                      : "Xóa phiếu này"
                  }
                >
...
const InputRow = ({
  ...
  <div className="grid grid-cols-[1fr_90px] md:grid-cols-[1fr_130px] gap-2 items-center group">
    <span
      className="text-xs md:text-sm text-slate-600 group-hover:text-slate-900 transition-colors truncate"
      title={label}
    >
      {label}
    </span>
    <SmartInput
      value={value}
      onCommit={onChange}
      className={`w-full text-right h-8 rounded border border-slate-200 px-2 text-xs md:text-sm font-medium text-slate-700 outline-none transition-all ${
        readOnly
          ? "bg-slate-50 cursor-not-allowed"
          : "bg-white focus:border-slate-400 focus:ring-1 focus:ring-slate-100"
      }`}
      readOnly={readOnly}
    />
  </div>
