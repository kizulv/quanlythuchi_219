...
export const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setIsUploading(true);
      try {
        // Generate immediate preview from processed canvas
        const preview = await getProcessedDataUrl(file);
        setImageUrl(preview);

        // Upload in background and replace preview with saved server URL when done
        const processedUrl = await processAndUploadImage(
          file,
          transaction.date
        );
        // Wait until the server file is actually loadable before swapping the preview
        const waitForImage = (url: string, timeout = 2000) => {
        ...
        // Try a few times (backoff) to allow the file to become readable by the static server
        let swapped = false;
        const attempts = [200, 400, 800, 1600];
        for (let i = 0; i < attempts.length; i++) {
          try {
            await waitForImage(processedUrl, attempts[i] + 500);
        ...
        if (!swapped) {
          // final attempt: set URL anyway (may be cached on later reload)
          setImageUrl(processedUrl);
        }
      } catch (error) {
          ...
          <div className="p-4 border-b flex justify-between items-center bg-slate-50">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Đơn vị nhập liệu: Nghìn đồng (VD: 13.400)
              </p>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <select
                  ...
                  onChange={(e) =>
                    setBreakdown({ ...breakdown, busId: e.target.value })
                  }
                >
            ...
            <div>
              ...
              <div className="space-y-3 pl-2">
                <InputRow
                  ...
                  onChange={(v) => handleNumberChange("revenueDown", v)}
                  displayFormatter={formatForDisplay}
                />
                <InputRow
                  label="- Ngược"
                  value={breakdown.revenueUp}
                  onChange={(v) => handleNumberChange("revenueUp", v)}
                  displayFormatter={formatForDisplay}
                />
                <InputRow
                  label="- Thu khác"
                  value={breakdown.revenueOther}
                  onChange={(v) => handleNumberChange("revenueOther", v)}
                  displayFormatter={formatForDisplay}
                />
              </div>
            ...
            <div>
              ...
              <div className="space-y-3 pl-2">
                <InputRow
                  ...
                  onChange={(v) => handleNumberChange("expenseFuel", v)}
                  displayFormatter={formatForDisplay}
                />
                <InputRow
                  label="- Chi cố định"
                  value={breakdown.expenseFixed}
                  onChange={(v) => handleNumberChange("expenseFixed", v)}
                  displayFormatter={formatForDisplay}
                />
                <InputRow
                  label="- Chi luật (CA)"
                  value={breakdown.expensePolice}
                  onChange={(v) => handleNumberChange("expensePolice", v)}
                  displayFormatter={formatForDisplay}
                />
                <InputRow
                  label="- Sửa chữa, bảo dưỡng"
                  value={breakdown.expenseRepair}
                  onChange={(v) => handleNumberChange("expenseRepair", v)}
                  displayFormatter={formatForDisplay}
                />
                <InputRow
                  label="- Chi khác"
                  value={breakdown.expenseOther}
                  onChange={(v) => handleNumberChange("expenseOther", v)}
                  displayFormatter={formatForDisplay}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm font-semibold">
                  Tổng dư sau chia{" "}
                  {!breakdown.isShared ? "(1 Xe: /2)" : "(2 Xe)"}:
                </span>
                <span className="font-bold text-lg">
